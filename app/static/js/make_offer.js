...
document.addEventListener('click', async function (e) {
    if (e.target.classList.contains('submitOfferBtn')) {
        e.preventDefault();
        ...
        try {
            ...
            const contract = new ethers.Contract(marketplace_contract, contractAbi, signer);

            const tx = await contract.proposeNFTPrice(
                contractAddress,
                tokenId,
                proposedPriceWei,
                { value: proposedPriceWei } // Still MON since MON is native
            );
